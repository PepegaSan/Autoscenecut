...
def load_cfg():
    global _CFG_MT, _CFG_OBJ
    try:
        mt = os.path.getmtime(CFG_PATH)
    except OSError:
        mt = -1.0
    if _CFG_OBJ is not None and _CFG_MT == mt:
        return _CFG_OBJ
    cfg = configparser.ConfigParser()
    cfg.read(CFG_PATH, encoding='utf-8')
    _CFG_MT, _CFG_OBJ = mt, cfg
    return cfg
...
_DAVINCI_PICK_KEY = None
_DAVINCI_PICK_VAL = None


def pick_davinci_worker_python(cfg):
    """Return a python.exe that can import DaVinciResolveScript (cached per INI mtime + paths + frozen)."""
    global _DAVINCI_PICK_KEY, _DAVINCI_PICK_VAL
    try:
        mt = os.path.getmtime(CFG_PATH)
    except OSError:
        mt = -1.0
    key = (
        mt,
        cfg.get('Settings', 'resolve_api_path', fallback='').strip(),
        cfg.get('Settings', 'davinci_python_path', fallback='').strip(),
        (os.environ.get('DAVINCI_SCRIPT_PYTHON') or '').strip(),
        bool(getattr(sys, 'frozen', False)),
    )
    if _DAVINCI_PICK_VAL is not None and _DAVINCI_PICK_KEY == key:
        return _DAVINCI_PICK_VAL
    out = _pick_davinci_worker_python_uncached(cfg)
    _DAVINCI_PICK_KEY, _DAVINCI_PICK_VAL = key, out
    return out


def _pick_davinci_worker_python_uncached(cfg):
    """
...
def _is_ntsc_fps(fps):
    f = float(fps)
    return abs(f - 29.97002997002997) < 0.05 or abs(f - 59.94005994005994) < 0.05


def _media_frames_from_sec(sec, fps):
    """Frame index in the source file (same convention as OpenCV int(sec * fps) seek)."""
    f = float(fps)
    if abs(f - 29.97002997002997) < 0.05:
        return int(float(sec) * 30000.0 / 1001.0)
    if abs(f - 23.976023976023978) < 0.01:
        return int(float(sec) * 24000.0 / 1001.0)
    if abs(f - 59.94005994005994) < 0.05:
        return int(float(sec) * 60000.0 / 1001.0)
    return int(float(sec) * f)


def _edit_frames_from_sec(sec, timebase, ntsc):
    """FCP7 timeline edit units; NTSC uses timebase 30 so 5:00 = frame 9000 in Resolve."""
    if ntsc == 'TRUE':
        return int(round(float(sec) * int(timebase)))
    return _media_frames_from_sec(sec, timebase)


def _frames_from_sec(sec, fps):
    return _media_frames_from_sec(sec, fps)
...
def _video_duration_sec(vid, fps):
    cap = cv2.VideoCapture(vid)
    try:
        fps_cap = cap.get(cv2.CAP_PROP_FPS) or float(fps) or 30.0
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
    finally:
        cap.release()
    if fps_cap <= 0:
        fps_cap = float(fps) if float(fps) > 0 else 30.0
    dur = (float(total_frames) / float(fps_cap)) if total_frames > 0 else 0.0
    if dur <= 0.0:
        dur = 1.0 / float(fps_cap)
    return dur


def _full_video_segment(vid, fps):
    """Single full-length segment (0..duration) for no-cuts XML/EDL export."""
    return [(0.0, _video_duration_sec(vid, fps))]


def segments_full_timeline_analysis(segment_results, duration_sec):
    """All analysis intervals on the source timeline (nothing removed, cuts between segments)."""
    if not segment_results:
        return [(0.0, duration_sec)] if duration_sec > 0 else []
    out = []
    for s in sorted(segment_results, key=lambda x: float(x.get('start', 0))):
        try:
...
def segments_full_timeline_from_merged(merged, duration_sec):
    """Split at keep-region edges so gaps stay on the timeline (fallback without segment_results)."""
    if duration_sec <= 0:
        return []
    boundaries = {0.0, float(duration_sec)}
    for s, e in merged or []:
...
def export_xml_xmeml(vid, segs, fps, out_dir, *, at_source_time=False):
    ...
    xml_name = f'{base_name}_scenecut_full.xml' if at_source_time else f'{base_name}_scenecut.xml'
    ...
    ntsc_edit = ntsc == 'TRUE'
    for s, e in segs:
        fi = _media_frames_from_sec(s, fps)
        fo = _media_frames_from_sec(e, fps)
        if fo <= fi:
            continue
        if at_source_time and ntsc_edit:
            t0 = _edit_frames_from_sec(s, tb, ntsc)
            t1 = _edit_frames_from_sec(e, tb, ntsc)
            total_end = max(total_end, t1)
        elif at_source_time:
            t0, t1 = fi, fo
            total_end = max(total_end, t1)
        else:
            if ntsc_edit:
                t0 = timeline_f
                t1 = timeline_f + (_edit_frames_from_sec(e, tb, ntsc) - _edit_frames_from_sec(s, tb, ntsc))
            else:
                t0 = timeline_f
                t1 = timeline_f + (fo - fi)
            timeline_f = t1
            total_end = timeline_f
        clip_elems.append((t0, t1, fi, fo))
    ...
    ET.SubElement(seq, 'name').text = 'Scenecut_full' if at_source_time else 'Scenecut'
    ...
    with open(xml_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))
    label = 'XML (FCP7 xmeml, volle Länge)' if at_source_time else 'XML (FCP7 xmeml)'
    print(f'{label} geschrieben: {xml_path}', flush=True)
    return True


def export_edl_cmx(vid, segs, fps, out_dir, *, at_source_time=False):
    ...
    abs_vid = os.path.abspath(vid)
    base_name = os.path.splitext(os.path.basename(abs_vid))[0]
    edl_name = f'{base_name}_scenecut_full.edl' if at_source_time else f'{base_name}_scenecut.edl'
    edl_path = os.path.join(out_dir, edl_name)
    ...
    title = 'Scenecut_full_' if at_source_time else 'Scenecut_'
    lines = ['TITLE: ' + title + base_name.replace(' ', '_'), f'FCM: {fcm}', '']
    ev = 1
    timeline_f = 0
    ntsc_edit = ntsc
    for s, e in segs:
        ...
        if at_source_time and ntsc_edit:
            rec_in = _frames_to_tc(_edit_frames_from_sec(s, tb, 'TRUE'), tb)
            rec_out = _frames_to_tc(_edit_frames_from_sec(e, tb, 'TRUE'), tb)
        elif at_source_time:
            rec_in, rec_out = src_in, src_out
        else:
            if ntsc_edit:
                dur = _edit_frames_from_sec(e, tb, 'TRUE') - _edit_frames_from_sec(s, tb, 'TRUE')
            else:
                dur = fo - fi
    ...
    with open(edl_path, 'w', encoding='utf-8', newline='\n') as f:
        f.write('\n'.join(lines))
    label = 'EDL (volle Länge)' if at_source_time else 'EDL'
    print(f'{label} geschrieben: {edl_path}', flush=True)
    return True
...
def _segment_csv_threshold_meta(cfg):
    """Flat tuning columns repeated on each row so spreadsheets are self-describing."""
    m = {}
    if cfg.has_section('Thresholds'):
        for k in cfg.options('Thresholds'):
            m[f'ini_{k}'] = cfg.get('Thresholds', k)
    m['ini_action_motion_target'] = cfg.get('Settings', 'action_motion_target', fallback='')
    m['ini_whisper_model'] = cfg.get('Settings', 'whisper_model', fallback='')
    m['ini_yamnet_enabled'] = cfg.get('Settings', 'yamnet_enabled', fallback='')
    return m


def write_segment_analysis_csv(vid, step, csv_rows, cfg):
    """
    If Settings.save_segment_analysis_csv=true, write one CSV under output/segment_logs/
    (same columns every run — compare runs on the same video by start_sec / segment_idx).
    """
    if not cfg.getboolean('Settings', 'save_segment_analysis_csv', fallback=False):
        return None
    if not csv_rows:
        return None
    out_dir = Path('output') / 'segment_logs'
    out_dir.mkdir(parents=True, exist_ok=True)
    stem = Path(vid).stem
    safe = ''.join((c if c.isalnum() or c in '._-' else '_') for c in stem)[:120] or 'video'
    ts = datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    path = out_dir / f'{safe}_seg{step}s_{ts}.csv'
    run_utc = datetime.now(timezone.utc).isoformat()
    vid_abs = os.path.abspath(vid)
    keep_meta = {
        'keep_dialogue': str(cfg.getboolean('Categories', 'keep_dialogue', fallback=False)).lower(),
        'keep_action': str(cfg.getboolean('Categories', 'keep_action', fallback=False)).lower(),
        'keep_vocal': str(cfg.getboolean('Categories', 'keep_vocal', fallback=False)).lower(),
        'keep_music': str(cfg.getboolean('Categories', 'keep_music', fallback=False)).lower(),
        'keep_silence': str(cfg.getboolean('Categories', 'keep_silence', fallback=False)).lower(),
    }
    thr_meta = _segment_csv_threshold_meta(cfg)
    fieldnames = [
        'run_utc',
        'video_path',
        'interval_seconds',
        *sorted(keep_meta.keys()),
        *sorted(thr_meta.keys()),
        'segment_idx',
        'start_sec',
        'end_sec',
        'final_category',
    ...
    with open(path, 'w', newline='', encoding='utf-8') as f:
        ...
        for row in csv_rows:
            ...
            w.writerow(out)
    ap = str(path.resolve())
    print(f'SEGMENT_CSV:{ap}', flush=True)
    return ap
...
def write_autocut_checkpoint(vid, merged, fps, step, out_dir, cfg_snapshot, segment_results=None, algo_snapshot=None):
    ...
    if algo_snapshot is not None:
        payload['algo_snapshot'] = algo_snapshot
...
# Brief pause before DaVinciResolveScript import (Davinci API start reference:
# reduces occasional fusionscript / binding init races when Resolve is busy.)
time.sleep(2)
...
try:
    ...
    for s, e in data['segs']:
        append_list.append({{
            'mediaPoolItem': clip,
            'startFrame': _media_frames_from_sec(s, data['fps']),
            'endFrame': _media_frames_from_sec(e, data['fps']),
        }})
...
def run_export_only(vid, merged, fps, out_dir, cfg, resolve_py=None, segment_results=None):
    ...
    no_cuts = 'NO CUTS' in engine_u
    cut_points = 'CUT POINTS' in engine_u
    at_source = cut_points and not no_cuts

    if no_cuts:
        segs = _full_video_segment(vid, fps)
    elif cut_points:
        dur = _video_duration_sec(vid, fps)
        if segment_results:
            segs = segments_full_timeline_analysis(segment_results, dur)
        else:
            segs = segments_full_timeline_from_merged(merged, dur)
        print(
            f'EXPORT_FULL_TIMELINE: segments={len(segs)} duration={dur:.2f}s (volle Länge, Schnitte an Segmentgrenzen)',
            flush=True,
        )
    else:
        segs = merged
    ...
    if 'AUTO-RENDER' in engine:
        if cut_points:
            print(
                'WARNUNG: „cut points – full length“ gilt nur für XML/EDL-Import — AUTO-RENDER exportiert die gekürzte Keep-Timeline.',
                flush=True,
            )
            segs = merged
        return render_davinci(vid, segs, api_path, fps, out_dir, cfg, resolve_py=resolve_py)
    if 'EDL' in engine:
        return export_edl_cmx(vid, segs, fps, out_dir, at_source_time=at_source)
    if 'XML' in engine:
        return export_xml_xmeml(vid, segs, fps, out_dir, at_source_time=at_source)
    return export_ffmpeg(vid, segs, preset, out_dir, cfg)
...
def retry_export_from_checkpoint():
    ...
    export_ok = run_export_only(vid, merged, fps, out_dir, cfg, segment_results=seg_res)
...
def main():
    ...
    for idx, t in enumerate(range(0, dur, step), start=1):
        ...
        csv_rows.append({
            ...
            'words': int(metrics.get('word_count', 0)),
            'wpm': '' if wpm is None else round(float(wpm), 4),
            'music': int(metrics.get('music_percent', 0)),
            'silence': int(metrics.get('silence_percent', 0)),
            'moan': int(metrics.get('moan_percent', 0)),
            'breath': int(metrics.get('breath_percent', 0)),
            'scream': int(metrics.get('scream_percent', 0)),
            'human_vocal': int(metrics.get('human_vocal_percent', 0)),
    ...
    algo_snapshot = build_algo_run_snapshot(cfg, video_path=os.path.abspath(vid), cfg_path=CFG_PATH)
    try:
        write_algo_run_artifact(algo_snapshot, vid, prefix='algo')
    except Exception as e:
        print(f'WARNUNG: Konnte algo snapshot nicht schreiben: {e}', flush=True)

    try:
        write_segment_analysis_csv(vid, step, csv_rows, cfg)
    except Exception as e:
        print(f'WARNUNG: segment CSV: {e}', flush=True)
    ...
    export_ok = run_export_only(
        vid, merged, fps, out_dir, cfg, resolve_py=resolve_py, segment_results=segment_results
    )
