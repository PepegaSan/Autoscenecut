...
# Avoid re-reading / re-parsing INI on every segment (hot path); invalidate via mtime.
_CFG_CACHE_MT = None
_CFG_CACHE_OBJ = None
_RUNTIME_CACHE_MT = None
_RUNTIME_CACHE_DICT = None

# Optional cProfile for analyze_segment: SCENECUT_PROFILE=1|yes|first (first segment), N (first N), all (every segment)
_SCENECUT_PROF_PARSED = False
_SCENECUT_PROF_MODE = 'off'  # off | all | count
_SCENECUT_PROF_LEFT = 0


def _scenecut_profile_should_run():
    """Return True if this analyze_segment call should run under cProfile."""
    global _SCENECUT_PROF_PARSED, _SCENECUT_PROF_MODE, _SCENECUT_PROF_LEFT
    if not _SCENECUT_PROF_PARSED:
        _SCENECUT_PROF_PARSED = True
        raw = (os.environ.get('SCENECUT_PROFILE') or '').strip().lower()
        if not raw or raw in ('0', 'off', 'false', 'no'):
            _SCENECUT_PROF_MODE = 'off'
        elif raw == 'all':
            _SCENECUT_PROF_MODE = 'all'
        elif raw.isdigit():
            _SCENECUT_PROF_MODE = 'count'
            _SCENECUT_PROF_LEFT = max(0, int(raw))
        elif raw in ('1', 'yes', 'true', 'first'):
            _SCENECUT_PROF_MODE = 'count'
            _SCENECUT_PROF_LEFT = 1
        else:
            _SCENECUT_PROF_MODE = 'off'
    if _SCENECUT_PROF_MODE == 'off':
        return False
    if _SCENECUT_PROF_MODE == 'all':
        return True
    if _SCENECUT_PROF_LEFT <= 0:
        return False
    _SCENECUT_PROF_LEFT -= 1
    return True
...
def load_config():
    global _CFG_CACHE_MT, _CFG_CACHE_OBJ
    try:
        mt = os.path.getmtime(CFG_PATH)
    except OSError:
        mt = -1.0
    if _CFG_CACHE_OBJ is not None and _CFG_CACHE_MT == mt:
        return _CFG_CACHE_OBJ
    cfg = configparser.ConfigParser()
    cfg.read(CFG_PATH, encoding='utf-8')
    _CFG_CACHE_MT, _CFG_CACHE_OBJ = mt, cfg
    return cfg
...
def get_runtime_settings():
    global _RUNTIME_CACHE_MT, _RUNTIME_CACHE_DICT
    try:
        mt = os.path.getmtime(CFG_PATH)
    except OSError:
        mt = -1.0
    if _RUNTIME_CACHE_DICT is not None and _RUNTIME_CACHE_MT == mt:
        return _RUNTIME_CACHE_DICT
    cfg = load_config()
...
def weighted_group_score(classnames, peak_scores, mean_scores, terms, exact_only=False, settings=None):
    if settings is None:
...
def get_yamnet_group_scores_from_audio(audio_path, settings=None):
    if settings is None:
        settings = get_runtime_settings()
    empty = {
        'speech_like': 0,
    ...
    try:
        ...
        speech_strict = weighted_group_score(classnames, peak_scores, mean_scores, speech_terms_strict, settings=settings)
        speech_broad = weighted_group_score(classnames, peak_scores, mean_scores, speech_terms_broad, settings=settings)
        music_like = weighted_group_score(classnames, peak_scores, mean_scores, music_terms, settings=settings)
        silence_like = weighted_group_score(classnames, peak_scores, mean_scores, silence_terms, settings=settings)
        moan_like = weighted_group_score(classnames, peak_scores, mean_scores, moan_terms, settings=settings)
        breath_like = weighted_group_score(classnames, peak_scores, mean_scores, breath_terms, settings=settings)
        scream_like = weighted_group_score(classnames, peak_scores, mean_scores, scream_terms, settings=settings)
        human_vocal_like = weighted_group_score(classnames, peak_scores, mean_scores, human_vocal_terms, settings=settings)
...
def get_motion_score(video_path, start_time, duration, settings=None):
    if settings is None:
        settings = get_runtime_settings()
...
def _analyze_segment_impl(video_path, start_time, duration=20):
    settings = get_runtime_settings()
    audio_path = extract_audio(video_path, start_time, duration)
    try:
        words = get_word_count_from_audio(audio_path)
        motion = get_motion_score(video_path, start_time, duration, settings)
        yam = get_yamnet_group_scores_from_audio(audio_path, settings)
...
def analyze_segment(video_path, start_time, duration=20):
    """If SCENECUT_PROFILE is set, run cProfile for selected segment(s) and print cumtime table."""
    if not _scenecut_profile_should_run():
        return _analyze_segment_impl(video_path, start_time, duration)
    import cProfile
    import io
    import pstats
    ...
    try:
        ...
        print('=== SCENECUT_PROFILE analyze_segment (cumulative, top 40) ===', flush=True)
        print(buf.getvalue(), flush=True)
        dump = (os.environ.get('SCENECUT_PROFILE_OUT') or '').strip()
        if dump:
            try:
                pr.dump_stats(dump)
                print(f'SCENECUT_PROFILE_OUT: binary stats written to {dump}', flush=True)
            except OSError as e:
                print(f'SCENECUT_PROFILE_OUT dump failed: {e}', flush=True)
